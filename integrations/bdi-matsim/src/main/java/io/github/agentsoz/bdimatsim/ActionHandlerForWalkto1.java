...
public final class ActionHandlerForWalkto1 implements BDIActionHandler {
	private static final Logger log = LoggerFactory.getLogger(ActionHandlerForWalkto1.class ) ;

	private final MATSimModel model;
	public ActionHandlerForWalkto1(MATSimModel model ) {
//		log.setLevel(Level.DEBUG);
	...
	public ActionContent.State handle(String agentID, String actionID, Object[] args) {
		log.debug("------------------------------------------------------------------------------------------");
		{
			...
			if (args.length < 4) {
				log.error("agent " + agentID + " WALKTO handler has " +args.length + " args (>=4 expected); will continue without handling this event");
		...
		switch (((MATSimModel.RoutingMode) args[3])) {
			case sOneFree:
				routingMode = MATSimModel.RoutingMode.sOneFree.name() ;
				break;
			case sOneGlobal:
				routingMode = MATSimModel.RoutingMode.sOneGlobal.name() ;
				break;
			default:
		...
		// new destination
		Activity newAct = model.getReplanner().editPlans().createFinalActivity( "walkTo", newLinkId ) ;
